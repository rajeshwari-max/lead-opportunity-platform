...
MANIFESTS: dict[str, SourceContract] = {
    ...
    "clean_air_fund": _c(
        ...
        known_defect=(
            "THE URL IS CORRECT AND THE PAGE IS THE WRONG KIND OF PAGE.\n"
            "\n"
            "This note used to read 'FETCHES NOTHING … the most likely cause "
            "is that it has moved', from a verification run that got 0 pages. "
            "The production log disproves that, and the wrong note is worse "
            "than no note — it sends the next person hunting for a moved URL. "
            "The 2026-08-24 scrape:\n"
            "\n"
            "    [Clean Air Fund] pagination exhausted at page 3 — every "
            "available page was scraped\n"
            "    [Clean Air Fund] completed — 5 found = 0 new saved + 5 "
            "already in database\n"
            "\n"
            "It fetches, it walks three pages, and it returns five rows whose "
            "deadlines had all passed months earlier (2026-01-09, 2026-03-09, "
            "2026-07-09). Page 2's three rows were all duplicates of page 1.\n"
            "\n"
            "So there are two things here and only one of them is a bug:\n"
            "  * Pagination may not be advancing — page 2 re-served page 1. "
            "The generic scraper now detects that and says so rather than "
            "reporting 'every available page was scraped'.\n"
            "  * /our-grants/ is a PORTFOLIO. It lists money already given, "
            "which is why excluded_types drops CONTRACT_AWARD and why a source "
            "pointed at it yields almost nothing storable. That is the page "
            "behaving as designed, not a scraper fault, and no parser change "
            "reaches it.\n"
            "\n"
            "The open question is whether Clean Air Fund publishes open calls "
            "on a different page at all. Answer it with evidence, not a guess: "
            "'python scripts/find_listing_url.py clean_air_fund' scores every "
            "candidate on rows/open/dated/awarded. A result of "
            "'rows 5 / open 0 / awarded 5' across every candidate means this "
            "funder publishes no open calls, and the honest action is to "
            "disable the source rather than keep scraping a portfolio."
        ),
    ),
    "fundsforngos": _c(

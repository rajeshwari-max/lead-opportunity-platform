...
let savedIds: Map<number, number> | null = null;
const savedListeners = new Set<()=>void>();
function publishSaved(rows:Lead[]) {
 savedIds = new Map(rows.map(l=>[l.opportunity.id,l.id]));
 savedListeners.forEach(fn=>fn());
}
function subscribeSaved(fn:()=>void) {savedListeners.add(fn);return ()=>{savedListeners.delete(fn);};}
export function SaveLeadButton({id}:{id:number}) {
 const ids=useSyncExternalStore(subscribeSaved,()=>savedIds);
 const leadId=ids?.get(id);
 const [busy,setBusy]=useState(false),[message,setMessage]=useState("");
 useEffect(()=>setMessage(""),[id]);
 return <span><button type="button" className="dl-save-button" disabled={busy||!ids} onClick={async()=>{
 setBusy(true);setMessage("");
 try {
 if(leadId!==undefined) await request(`/my-leads/${leadId}`,"DELETE");
 else await request("/my-leads","POST",{opportunity_id:id});
 publishSaved(await request("/my-leads"));
 setMessage(leadId!==undefined?"Removed from My leads; progress retained":"Saved to My leads");
 window.dispatchEvent(new Event("saved-leads-changed"));
 }catch(e){setMessage(e instanceof Error?e.message:"Could not update saved lead");}finally{setBusy(false);}
 }}>{busy?"Updating…":leadId!==undefined?"Unsave":"Save lead"}</button><small role="status"> {message}</small></span>;
}
...
export function DashboardLeads({isAdmin=false,name=""}:{isAdmin?:boolean;name?:string}) {
 ...
 const reload=async()=>{const [saved,stats]=await Promise.all([request("/my-leads"),request("/my-leads/activity")]);setLeads(saved);publishSaved(saved);setSelected(current=>current&&saved.some((l:Lead)=>l.id===current.id)?current:null);setActivity(stats);};
 useEffect(()=>{void run(reload);const listener=()=>void run(reload);const hash=()=>{if(location.hash==="#my-leads")setOpen(true);};window.addEventListener("hashchange",hash);window.addEventListener("saved-leads-changed",listener);return()=>{window.removeEventListener("saved-leads-changed",listener);window.removeEventListener("hashchange",hash);savedIds=null;savedListeners.forEach(fn=>fn());};},[]);
 ...
 {selected&&<form onSubmit={e=>{e.preventDefault();void run(async()=>{const {stage,notes,factors,next_action,next_action_date}=selected;await request(`/my-leads/${selected.id}`,"PUT",{stage,notes,factors,next_action,next_action_date:next_action_date||null});await reload();setEvents(await request(`/my-leads/${selected.id}/history`));setNotice("Progress saved to your account");});}}><fieldset disabled={busy}><h3>{selected.opportunity.title}</h3><SaveLeadButton id={selected.opportunity.id}/><p>Unsave removes this lead from your list and keeps its notes and progress for later.</p><label>Progress<select value={selected.stage} onChange={e=>setSelected({...selected,stage:e.target.value})}>{stages.map(s=><option key={s}>{s}</option>)}</select></label><label>Notes<textarea maxLength={20000} value={selected.notes} onChange={e=>setSelected({...selected,notes:e.target.value})}/></label><label>Next step<input maxLength={2000} value={selected.next_action} onChange={e=>setSelected({...selected,next_action:e.target.value})}/></label><label>Follow-up date<input type="date" value={selected.next_action_date||""} onChange={e=>setSelected({...selected,next_action_date:e.target.value})}/></label><button>Save progress</button></fieldset><ul>{events.map(v=><li key={v.id}>{v.stage} · {v.note} · {v.created_at.slice(0,10)}</li>)}</ul></form>}</div></>}
